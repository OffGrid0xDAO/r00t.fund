...
contract OpenCactus is Script {
    ...
    function run() external {
        ...
        bytes32 id = bytes32("CACTUS-RAISE");
        address treasury = address(uint160(uint256(keccak256("r00t.regen.treasury.cactus"))));

        vm.startBroadcast(pk);
        // new parcel token to auction
        TestToken cactus = new TestToken("Cactus Parcel", "CACTUS");
        cactus.mint(me, 250_000e18);
        cactus.approve(LAUNCHPAD, type(uint256).max);

        // open a 2-hour uniform-price CCA: 100k CARROT sold, 100k for pools, floor 0.01 R00T/CARROT
        ILaunchpad(LAUNCHPAD).createParcel(id, IERC20(address(cactus)), treasury, 100_000e18, 100_000e18, 0.01e18, 7200);
        ...
        console2.log("opened CACTUS raise; phase:", ILaunchpad(LAUNCHPAD).phaseOf(id));
        console2.log("CACTUS token:", address(cactus));
    }
}
