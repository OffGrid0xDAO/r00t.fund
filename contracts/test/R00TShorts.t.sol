...
contract R00TShortsTest is Test {
    ...
    function test_OpenShort_RevertOpenInterestLimit() public {
        // Contract default OI cap is now 50% (DEFAULT_MAX_OPEN_INTEREST_BPS = 5000).
        // Set it to 10% for this test so the cap is reachable in a few positions.
        // At ~10,000 tokens/ETH, 1 ETH collateral shorts ~10,000 tokens → 10% of 1M = 100k.
        shorts.setMaxOpenInterestBps(1000);
    ...
    function test_Liquidate_Success() public {
        ...
        // maxRepurchaseCost must be a real cap now (Vuln 9 fix removed the `0 == no-limit`
        // bypass — 0 always reverts SlippageExceeded). Pass max to let the buyback execute.
        vm.prank(liquidator);
    ...
    function test_Liquidate_RevertNotOpen() public {
        ...
        // Try again — !isOpen is checked BEFORE slippage, so this reverts PositionNotOpen
        vm.prank(liquidator);
        vm.expectRevert(IR00TShorts.PositionNotOpen.selector);
        shorts.liquidate(positionId, type(uint256).max);
    }
