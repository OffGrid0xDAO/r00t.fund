...
///      Receives health reports from the CRE DON that read ZkAMMPair, R00TShorts,
...
contract ProtocolHealthMonitor is R00tCREReceiver {
    ...
    /// @notice ZkAMMAdmin address for emergency actions
    ...
    /// @param ethReserve Current ETH reserve in ZkAMMPair
    /// @param tokenReserve Current token reserve in ZkAMMPair
    ...
    function receiveReport(
        ...
        if (autoCircuitBreakerEnabled && uint8(risk) >= uint8(circuitBreakerThreshold)) {
            ...
            // Note: actual emergency pause requires calling ZkAMMAdmin
